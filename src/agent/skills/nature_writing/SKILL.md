...
- `skill_id`: **`nature_writing`**. Load `manifest.yaml` and `static/` fragments via `read_skill(..., relative_path=...)`.
- For polishing finished English prose, prefer `nature_polishing`.
