...
metadata:
  version: "6.1.0"
  skill-author: VenusFactory2 (Yuan1z skill, community)
---
...
- `skill_id`: **`nature_polishing`**. Load `manifest.yaml` and `static/` fragments via `read_skill(..., relative_path=...)`.
- For drafting new sections from claims/results, prefer `nature_writing`.
