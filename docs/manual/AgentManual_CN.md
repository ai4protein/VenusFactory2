...
| 想在执行前先看一眼计划 | 可以重排、改写或删除步骤 |
| 长任务希望分段确认 | 每个步骤 / 子报告都可"继续 / 重写 / 中止" |
...
| **中栏 — 对话区** | 消息时间线、模型选择器、文件上传 / Workspace 选择、Regenerate / Export / Stop / Send。需要你确认的"检查点卡片"会内嵌在这里。 |
...
典型一次任务会经历五个阶段，每个阶段 Agent 都会弹出对应的检查点卡片，让你在不微管的情况下保持掌控：

```
你 ─▶ 澄清 ─▶ 规划 ─▶ 执行 ─▶ 子报告确认 ─▶ 迭代决策 ─▶ 最终报告
```

### 3.1 澄清（Clarification）

Agent 可能会先问几个简短问题——单选、多选或文本输入。选择最符合你意图的选项（或在"其他"里填自由文本）。提交后进入下一步。

> **小技巧：** 初始消息里直接说明"跳过澄清"，简单任务可以直接进规划。
...
### 3.4 子报告检查点（Sub-report）

每一个分析步骤完成后，Agent 会写一个短的子报告，然后等你三选一：

- **Continue Research** — 接受当前结果，进入下一步
- **Comment & Rewrite** — 留言修改，Agent 会按你的反馈重写该子报告
- **Skip to Report** — 放弃这条分支，直接生成最终报告
